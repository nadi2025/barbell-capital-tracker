...
export default function ExpiryAlerts({ trades }) {
  ...
  // Upcoming open options expiring in next 30 days
  ...
  // Assignment log — assigned trades
  const assignments = trades.filter(t => t.status === "Assigned").sort((a, b) =>
    (b.close_date || b.expiration_date || "").localeCompare(a.close_date || a.expiration_date || "")
  );
      ...
      {upcoming.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs bg-amber-500/10 text-amber-600 border border-amber-500/20 px-2 py-0.5 rounded-full">{upcoming.length}</span>
          </div>
          ...
          {upcoming.map(t => {
            ...
            const stockCost = (t.strike || 0) * (t.quantity || 0) * 100;

            return (
              <div key={t.id} className={`rounded-xl border px-4 py-3 ${u.bg}`}>
                <div className="flex flex-wrap items-start gap-3 justify-between">
                  <div className="flex items-start gap-3">
                    <AlertTriangle className={`w-4 h-4 mt-0.5 shrink-0 ${u.icon}`} />
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-semibold text-sm font-mono">{t.ticker}</span>
                        <span className="text-xs text-muted-foreground">{t.category} · Strike ${t.strike} · Qty {t.quantity}</span>
                        <span className={`text-xs px-2 py-0.5 rounded-full border ${u.badge}`}>
                          {t.daysLeft === 0 ? "פוקע היום!" : `${t.daysLeft} ימים`}
                        </span>
                        <span className="text-xs text-muted-foreground font-mono">{t.expiration_date}</span>
                      </div>
                      <div className="flex gap-4 mt-1 text-xs text-muted-foreground">
                        <span>פרמיה שנגבתה: <span className="text-profit font-mono font-semibold">${premium.toLocaleString()}</span></span>
                        <span>עלות מניות אם ימומש: <span className="font-mono font-semibold">${stockCost.toLocaleString()}</span></span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {decision ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <button onClick={() => decide(t.id, null)} className="text-xs text-muted-foreground hover:text-foreground underline">שנה</button>
                      </div>
                    ) : (
                      <>
                        <button
                          onClick={() => decide(t.id, "assign")}
