...
// Post-process closed option fills. IB sometimes synthesizes a closing fill at
// the underlying's price (rather than the option's price) when a short option
// expires OTM. Detect those and reclassify as Expired with full-premium P&L.
//   Short Call w/ close_price < strike → call expired worthless → Expired
//   Short Put  w/ close_price > strike → put expired worthless → Expired
function reclassifyOtmFalseCloses(closedOpts) {
  for (const p of closedOpts) {
    if (p.status !== "Closed" || p.direction !== "Short") continue;
    const isCallOtm = p.category === "Call" && p.close_price < p.strike;
    const isPutOtm  = p.category === "Put"  && p.close_price > p.strike;
    if (!isCallOtm && !isPutOtm) continue;
    p.status = "Expired";
    p.expiredOtm = true;
    p.close_price = 0;
    p.pnl = p.avgFillPrice * p.initial_qty * OPT_MULT - p.totalCommission;
  }
}
...
export function reconcileCsv(csvText, today = new Date()) {
  ...
  // Post-process: reclassify "Closed" short options where close_price is
  // actually the underlying's market price at expiration (IB quirk for OTM).
  reclassifyOtmFalseCloses(closedOptions);
